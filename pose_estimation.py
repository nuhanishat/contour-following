...
class PoseEstimation():
	def __init__(self, marker_size = 60):
	...
	def compute_marker_pose(self, marker):
		...
		self.delta_corners = [x - y for x, y in zip(marker.corner1, marker.corner2)]
		
		if abs(self.delta_corners[1]) > 0: # if there is difference in y coord
			angle = math.atan(self.delta_corners[1]/self.delta_corners[0])
			marker.theta = angle # Update pose angle by this angle
		
		# Rotate center by angle and translate by corner 1
		marker.px = marker.corner1[0] + (local_center_x*np.cos(marker.theta) - local_center_y*np.sin(marker.theta))
		marker.py = marker.corner1[1] + (local_center_x*np.sin(marker.theta) + local_center_y*np.cos(marker.theta))


	def pixel_to_world(self, marker, width_resolution, depth_resolution):
		marker.x =  marker.px * width_resolution
		marker.y = marker.py * width_resolution
		marker.z = depth_resolution * max(self.delta_corners)
